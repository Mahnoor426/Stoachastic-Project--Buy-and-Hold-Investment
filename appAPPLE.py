...
from Post_processing import plot_price_with_time, plot_histogram, plot_kde, show_stats, print_percentiles

import LinearRegression
from flask_cors import CORS

 
# Path to the Apple dataset
apple_file_path = r'.\AAPL.xlsx'

# Run linear regression on the Apple dataset
apple_data = LinearRegression.run_linear_regression(apple_file_path)
app = Flask(__name__)
CORS(app)
...
# # Load the ETF data
ETF_data = pd.read_excel('AAPL_with_predictions.xlsx', index_col=0, parse_dates=['date'])
ETF_data.index.name = 'date'
ETF_data['return'] = (ETF_data['Predicted_Close'].pct_change()).shift(-1)
ETF_data['ln_return'] = (np.log(ETF_data['Predicted_Close'].shift(-1) / ETF_data['Predicted_Close']))
trading_days_per_year = int(ETF_data['Predicted_Close'].resample("Y").count()[:-1].mean())
delta_days = (ETF_data.index.date[-1] - ETF_data.index.date[0]).days
cumu_return = (ETF_data.Predicted_Close[-1] - ETF_data.Predicted_Close[0]) / ETF_data.Predicted_Close[0]
...
def run_simulation():
    ...
    # return jsonify({"message": "Simulations completed"})
     # Convert each DataFrame in Matrix_results to JSON-serializable format
    matrix_results_json = []
